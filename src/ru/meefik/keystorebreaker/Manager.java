...
public class Manager {
	...
	public Manager(String ksFile, char[] seq, char[] firstPwd, char[] lastPwd, int threads) {
		this.char_seq = seq;
		...
		try {
			...
			for (int i = 0; i < threads; i++) {
				...
				System.out.println("#"+i+": "+String.valueOf(fp)+" - "+String.valueOf(lp));
	        ...
	        while (alive) {
	        	...
	        	for (int i = 0; i < threads; i++) {
	            	...
	            	if (cp > lp || found) br[i].inactive();
	            	alive = br[i].isActive() || alive;
	            	
	            	
	        		System.out.println("#"+i+": "+String.valueOf(br[i].getPasswd())+" / "+String.valueOf(br[i].lastPwd)+"   "+
	        				String.valueOf((cp-fp)*100/(lp-fp))+"% "+
	        				"[ " + String.valueOf((cp-fp)*1000/(System.currentTimeMillis() - t)) + " pwd/s ]" );
	        	}

	        	speed = (sum*1000/(System.currentTimeMillis() - t));
	        	timeLeft = (amount - sum)/speed;  // sec
	        	
	        	System.out.print("Total: "+(sum*100/amount) + "% [ " + speed + " pwd/s ]  ");
	        	System.out.println("Time left: " + timeLeft/(24*60*60) + "." + df.format(new Date(timeLeft*1000)));
	        }
